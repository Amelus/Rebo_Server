...
##########################
# Extract subPath param

QUERY_PARAM=''

if [[ $FOLDERTOKEN == *"?path="* ]]; then
        pat='(.*)(\?path=\/)(.*)'
	[[ $FOLDERTOKEN =~ $pat ]]
	FOLDERTOKEN="${BASH_REMATCH[1]}"
	QUERY_PARAM="${BASH_REMATCH[3]}/"
fi
...
echo "$CURLBIN"$INSECURE$VERBOSE -T "$FILENAME" -u "$FOLDERTOKEN":"$PASSWORD" -H "$HEADER" "$CLOUDURL/$PUBSUFFIX/$QUERY_PARAM$BFILENAME"
"$CURLBIN"$INSECURE$VERBOSE -T "$FILENAME" -u "$FOLDERTOKEN":"$PASSWORD" -H "$HEADER" "$CLOUDURL/$PUBSUFFIX/$QUERY_PARAM$BFILENAME"
